...
# Jenkins 需将 mall-admin-1.0-SNAPSHOT.jar 与本目录 Dockerfile 传到 deploy_dir（见下方变量）。
#
set -euo pipefail
...
if [[ ! -f "${deploy_dir}/mall-admin-${version}.jar" ]]; then
  echo "错误：未找到 ${deploy_dir}/mall-admin-${version}.jar，请先由 Jenkins 上传 JAR"
  exit 1
fi
